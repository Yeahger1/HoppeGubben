...
public class planeMoveScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (rb2D == null)
        {            
            velocity += (Vector2.left * moveSpeed * 0.005f) * Time.deltaTime;
        }
        position = transform.position;
        velocity += (Vector2.left * moveSpeed * 0.005f) * Time.deltaTime;

        //*-1 if gravityField is turned off
        if (power.gravityFieldOn && power.gravityFieldPosition.sqrMagnitude < transform.position.sqrMagnitude) //|| power.gravityFieldPosition.y +2 < transform.position.y || power.gravityFieldPosition.x -2 > transform.position.x || power.gravityFieldPosition.y - 2 > transform.position.x)
        {
